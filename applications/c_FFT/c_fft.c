...
void FFT(Complex *A, Complex *a, Complex *W, unsigned N,           
  ...
  //doubleType start = omp_get_wtime();
#pragma omp parallel for
    for(i = 0; i <= 1; i++) {
      FFT(D + i * n, a + i * stride, W, n, stride << 1, A + i * n);
    }
	//doubleType finish = omp_get_wtime();
//printf("First version time usage =%g\n",(finish-start));
    /* Combination stage */
    B = D;
    C = D + n;
   //start = omp_get_wtime();
...
#pragma omp parallel for default(none) private(i, Aux, pW) shared(stride, n, A, B, C, W)
  ...
  //finish = omp_get_wtime();
	//printf("Second OMP version time usage =%g\n",(finish-start));
  }
}
/* ----------------------------------------------------------------------- */
unsigned get_params(int argc, char *argv[]) {
...
int main(int argc, char *argv[]) {
  ...
  /* write_array(N, A); */
	...
	if (test_array(N, A))
		...
		printf("Fails\n");
  printf("OMP version time usage =%g\n",(finish-start));
